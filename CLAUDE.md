<!-- SPECKIT START -->
**Active feature**: 027 (aggressive-bracket-fill) — **AWAITING BROWSER SMOKE** on branch `027-aggressive-bracket-fill` (2026-05-07). Implementation complete: `taxOptimizedWithdrawal` extended with `options.disableSmoothingCap` flag (both HTMLs); new `AGGRESSIVE_BRACKET_FILL` strategy registered as 8th entry in `STRATEGIES` (color `#fb923c` orange-amber); 4 i18n keys × 2 languages × 2 HTMLs = 16 new translation strings + Translation Catalog updated. **Tests:** 493/493 unit tests passing (added `tests/unit/aggressiveBracketFill.test.js` 7 cases incl. SC-026-A pin within ±5%, and `tests/unit/aggressiveBracketFillRanker.test.js` 5 cases for ranker integration); `tests/e2e/aggressive-bracket-fill.spec.ts` 8/8 passing on Chromium against both HTMLs. SC-026-A pin: lifetime tax $116,507 / terminal BV $1,129,821 real-$ — both verified within ±5%. Constitution check: PASS, no Complexity Tracking entries. Merge gate: T030 manual browser smoke per CLAUDE.md (cycle Mode×Objective; verify red+purple Trad bars 60-69 ~$118K each year + ~9% effective tax under aggressive selection; toggle EN↔中文; confirm 8-row Strategy Ranking audit panel). Spec [`spec.md`](./specs/027-aggressive-bracket-fill/spec.md), Plan [`plan.md`](./specs/027-aggressive-bracket-fill/plan.md), Closeout [`CLOSEOUT.md`](./specs/027-aggressive-bracket-fill/CLOSEOUT.md). Predecessor 026 (withdrawal-tax-and-ui-fixes) merged to main 2026-05-07 via merge commit `ad7b0c4`. Predecessor 025 merged via `da49022` 2026-05-04.
