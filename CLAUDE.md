<!-- SPECKIT START -->
**Active feature**: 024 (deferred-fixes-cleanup) — **AWAITING USER BROWSER-SMOKE before merge to `main`**. All 6 user stories shipped (51 of 51 CLI-executable tasks; 5 user-side gated). Tests: **502 passing, 1 intentional skip, 0 failures** (was 501 baseline + 1 net new). **0 CRITICAL · 0 HIGH · 0 MEDIUM · 0 LOW** finding (B-022-1 fix expected to clear the residual E3 finding from feature 023; browser-smoke verifies). Constitution VIII gate (`spendingFloorPass.test.js`): 7/7 throughout. Bundle of 5 carry-forward backlog items: B-022-1 `_chartFeasibility` quantization (E3 LOW → 0), B-022-2 `scenario.tax.china` dedup (cleanup), B-022-3 Healthcare card Book Value (last frame leak closed), B-023-5 SS COLA decoupling (new `ssCOLARate` slider; default = `inflationRate` preserves behavior), B-023-6 sim reconciliation (refined `expected` annotation: clamping artifact when both sims feasible). Plus 023 docs drift cleanup in BACKLOG.md + CLOSEOUT.md. Spec [`spec.md`](./specs/024-deferred-fixes-cleanup/spec.md), Plan [`plan.md`](./specs/024-deferred-fixes-cleanup/plan.md), Tasks [`tasks.md`](./specs/024-deferred-fixes-cleanup/tasks.md), Audit Report [`audit-report.md`](./specs/024-deferred-fixes-cleanup/audit-report.md), CLOSEOUT [`CLOSEOUT.md`](./specs/024-deferred-fixes-cleanup/CLOSEOUT.md). Predecessor 023 merged to main 2026-05-02 (merge commit `9c08b4c`).
