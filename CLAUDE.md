<!-- SPECKIT START -->
**Active feature**: 023 (accumulation-spend-separation) — **AWAITING USER BROWSER-SMOKE before merge to `main`**. All 9 implementation phases complete (54 of 54 CLI-executable tasks; 5 user-side tasks T055–T059 gated). Tests: **501 passing, 1 intentional skip, 0 failures** (was 478 baseline + 23 net new). **0 CRITICAL · 0 HIGH · 0 MEDIUM · 1 LOW** finding (E3 residual `RR-pessimistic-frugal` unchanged from B-022-1; independent of accumulation-spend plumbing). Constitution VIII gate (`spendingFloorPass.test.js`): 7/7 throughout. Latent calc bug surfaced during 022 user-validation now CLOSED: RR-baseline year-1 portfolio Book Value Δ drops from **+$191,722** to **~+$96,851** (49% reduction; SC-001 satisfied). 6 callers of `accumulateToFire` thread the new `options.accumulationSpend` field via `resolveAccumulationOptions` across both HTMLs (12 sites). Caller #6 refactor (was using `{}` empty options) corrected mid-implementation per Phase 0 R2 audit. 2 new audit invariant families (`country-tier-isolation`, `accumulation-spend-consistency`) lock the contract. Spec [`spec.md`](./specs/023-accumulation-spend-separation/spec.md), Plan [`plan.md`](./specs/023-accumulation-spend-separation/plan.md), Tasks [`tasks.md`](./specs/023-accumulation-spend-separation/tasks.md), Audit Report [`audit-report.md`](./specs/023-accumulation-spend-separation/audit-report.md), CLOSEOUT [`CLOSEOUT.md`](./specs/023-accumulation-spend-separation/CLOSEOUT.md). Predecessor 022 merged to main 2026-05-01 (merge commit `3d67c9c`).
