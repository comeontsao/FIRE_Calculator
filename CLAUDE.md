<!-- SPECKIT START -->
**Active feature**: 024 (deferred-fixes-cleanup) — **AWAITING USER BROWSER-SMOKE before merge to `main`** (smoke needs re-running after 2026-05-03 scope expansion). 9 user stories shipped (5 user-side gated). Tests: **503 passing, 1 intentional skip, 0 failures** (was 501 baseline + 2 net new — T7b for US5, B-024-2 regression for US8). **0 CRITICAL · 0 HIGH · 0 MEDIUM · 0 LOW** finding. Constitution VIII gate (`spendingFloorPass.test.js`): 7/7 throughout. Original scope (2026-05-02): 5 carry-forward backlog items (B-022-1 quantization, B-022-2 china dedup, B-022-3 healthcare BV, B-023-5 SS COLA, B-023-6 sim reconciliation) + 023 docs drift. **Scope expansion (2026-05-03)** — added US7 (B-024-3 cash-first lump-sum bucket priority — addresses user's age-54 hiccup screenshot showing $269k stock drain while $260k cash sat idle), US8 (B-024-2 lump-sum unconditionally inhibited when sellAtFire=true — eliminates wasteful pre-FIRE payoff when home sale will discharge mortgage), US9 (KPI relabel "Current Net Worth" → "Whole Portfolio Net Worth"; value now sums all 4 buckets matching Lifecycle chart Total Portfolio at currentAge). Spec [`spec.md`](./specs/024-deferred-fixes-cleanup/spec.md), Plan [`plan.md`](./specs/024-deferred-fixes-cleanup/plan.md), Tasks [`tasks.md`](./specs/024-deferred-fixes-cleanup/tasks.md), Audit Report [`audit-report.md`](./specs/024-deferred-fixes-cleanup/audit-report.md), CLOSEOUT [`CLOSEOUT.md`](./specs/024-deferred-fixes-cleanup/CLOSEOUT.md). Predecessor 023 merged to main 2026-05-02 (merge commit `9c08b4c`).
