<!-- SPECKIT START -->
**Active feature**: 037 (lifecycle-excel-export) — **merged to main 2026-08-14** (commit `704c1f9`, fast-forward; branch deleted local + remote). **39/42 tasks ticked, 2 superseded (T033/T034), 1 genuinely open: T040 — the human "open it in real Excel" check was NOT done.** That makes three consecutive features (035, 036, 037) merged with the human gate unsigned; if Excel ever refuses the file or the 70 columns prove unnavigable, that's why. Verified at merge: unit **818/818**; full Playwright **220 pass / 1 fail** where the 1 passes 10/10 isolated (load-contention flake — a NEW member of the known family, characterised in [`BASELINE.md`](./specs/037-lifecycle-excel-export/BASELINE.md)); 037 E2E **20/20** both dashboards; `tools/verify-037-export.mjs` **42/42** against live RR numbers (**53/53** years match the *rendered* Chart.js values on RR, **60/60** on Generic; RR 2026 = $532,021 = the KPI card); console-probe 0 both; smoke-032 15/15. **Four planning-doc corrections are recorded in [`CLOSEOUT.md`](./specs/037-lifecycle-excel-export/CLOSEOUT.md) — in every case the written spec was wrong and the code was right**, incl. research R4 (claimed retirement tax existed nowhere; `mix.taxOwed` already had it, which cancelled the feature's only calc change) and data-model INV-3 (read `money >= purchasingPower`, which **inverts for negative values** — now sign-aware). **Recurring failure worth internalising**: four times, a true observation about *some* per-year array was generalised into a claim about *the* array. There are THREE (`perYearRows` = what the export handler passes; `result.strategy`; `options._trajectory` = never pass, names the field `synth`, gets no companions) and they are NOT interchangeable — the wrong one yields a workbook quietly wrong in ~18 columns with nothing thrown. Now caught by the **INV-9 canary**. One button in **History → Snapshots** (beside the existing `📤 Export CSV`) downloads a real `.xlsx`: **one row per calendar year**, current year → plan end, carrying every per-year figure the Lifecycle projection computes, in **both** money and purchasing-power frames, matching the chart exactly. **Structural finding driving the work (research R2)**: `projectFullLifecycle` emits **two different row shapes** — accumulation rows have 27 fields incl. full cash-flow detail (RR ~11614), retirement rows have only 15 and **no cash-flow detail** (RR ~11884) — and withdrawals-by-source (`wTrad`/`wRoth`/…) live on the **strategy** rows entirely. So "all the numbers" = a **union of 3 sources joined by `age`** (~68 columns), with **blank ≠ zero** semantics (data-model §2). **Principle V exception, user-approved 2026-08-13**: adds **ExcelJS 4.4.0** (cdnjs, MIT, 926 KB, verified classic UMD / zero ESM syntax), **lazy-loaded on first click** so cold load is unchanged. SheetJS CE was rejected — freeze panes are Pro-only. Corrected during planning: the "must export offline" bar was incoherent (Chart.js is already CDN-only, so charts don't render offline either). **Known gap (research R4)**: retirement-year federal tax is computed inside `taxOptimizedWithdrawal` and **surfaced nowhere** — Phase F adds it as an additive sibling field, and is skippable. Architecture: pure `calc/lifecycleExport.js` (UMD const **`_lifecycleExportApi`, never `_api`**) does all the logic Node-testably; a thin browser shim writes bytes. Plan: [`specs/037-lifecycle-excel-export/plan.md`](./specs/037-lifecycle-excel-export/plan.md).
