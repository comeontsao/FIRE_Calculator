<!-- SPECKIT START -->
**Active feature**: 028 (strategy-aware-fire-age) — **IMPLEMENTATION COMPLETE — AWAITING BROWSER SMOKE** on branch `028-strategy-aware-fire-age` (2026-05-08). Two-layer fix shipped: (1) US1 stop-gap pill guard via `_shouldOverrideStatusToInfeasible` pure helper + `projectFullLifecycle` probe in `renderFireStatus()` — closes the misleading green "On Track" verdict immediately when chart shows depletion under non-default strategy; (2) US2 root-cause threading — `signedLifecycleEndBalance` and `simulateRetirementOnlySigned` extended with `options.strategyOverride` + `options.thetaOverride` mirroring `projectFullLifecycle`; resolver wrapper threads strategy via `getActiveChartStrategyOptions()`; (3) US3 audit visibility — `crossValidationWarnings.endBalance-mismatch` extended with `activeStrategyId`, `mode`, `chartEndBalance`, `signedEndBalance` fields. **Tests**: 528/528 unit (493 baseline + 35 new across 4 new test files) + 8/8 E2E on Chromium against both HTMLs. Constitution check: PASS on all 9 principles, no Complexity Tracking entries. Lockstep audit: RR +208 / Generic +207 lines (1-line delta = RR personal content). Merge gate: T035 manual browser smoke per CLAUDE.md (cycle Mode×Objective; verify pill flips from "On Track" to infeasible under SC-027 reproducer; confirm no false negatives on feasible scenarios; toggle EN↔中文; confirm `crossValidationWarnings` empty post-fix). Spec [`spec.md`](./specs/028-strategy-aware-fire-age/spec.md), Plan [`plan.md`](./specs/028-strategy-aware-fire-age/plan.md), Research [`research.md`](./specs/028-strategy-aware-fire-age/research.md), Contract [`contracts/signed-sim-options.contract.md`](./specs/028-strategy-aware-fire-age/contracts/signed-sim-options.contract.md), Quickstart [`quickstart.md`](./specs/028-strategy-aware-fire-age/quickstart.md), Closeout [`CLOSEOUT.md`](./specs/028-strategy-aware-fire-age/CLOSEOUT.md). Predecessor 027 (aggressive-bracket-fill) merged to main 2026-05-07 via merge commit `a316ed1`.
