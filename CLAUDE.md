<!-- SPECKIT START -->
**Active feature**: 034 (year-tax-estimator) — planning complete, on branch `034-year-tax-estimator`. RR-only single-year federal tax microscope at the bottom of the Withdrawal Strategy tab: year picker → auto-pull projected ordinary income + LTCG → correct LTCG-on-ordinary stacking (0%/15%/20%) in the selected year's nominal dollars, with "room left at 0% LTCG" headline, marginal/IRMAA/NIIT signals, show-your-work breakdowns. Local what-if only (no Lifecycle write-back). New pure UMD module `calc/taxEstimator.js`. Plan: [`specs/034-year-tax-estimator/plan.md`](./specs/034-year-tax-estimator/plan.md). Next: `/speckit-tasks`.
