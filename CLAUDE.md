<!-- SPECKIT START -->
**Active feature**: 033 (math-assumptions-cleanup) — **TASKS READY (T001–T027) — AWAITING /speckit-implement** on branch `033-math-assumptions-cleanup` (2026-06-05). [`tasks.md`](./specs/033-math-assumptions-cleanup/tasks.md): 27 tasks across Setup (baseline probe FIRST), Foundational (caller-audit + test-impact catalog), US1 cash dial (MVP), US2 funding ladder, US3 Fisher, Polish (FIRE-age delta CLOSEOUT + full merge gate). Stories sequential; per-surface edits within a story parallelize across Frontend/Backend/QA. Three math corrections in one wave: (1) new `calc/assumptions.js` defining `CASH_REAL_RETURN = 0.0` (clarification Q1 — cash tracks inflation; supersedes feature-030 FR-016's hardcoded ×1.005 at 9 sites/HTML + `calc/accumulateToFire.js:782` + `calc/getCanonicalInputs.js:239`), (2) honest funding ladder for negative accumulation residuals (cut stock contribution → draw cash → draw brokerage; sibling fields `stockContributionActual`/`fundedFromCash`/`fundedFromStocks`; conservation residual back to ≈ $0 from ≈ −$32K), (3) Fisher `realRate()` helper across ~28 subtraction sites/HTML + 4 calc sites (clarification Q2 — in scope). Spec docs: [`spec.md`](./specs/033-math-assumptions-cleanup/spec.md), [`plan.md`](./specs/033-math-assumptions-cleanup/plan.md), [`research.md`](./specs/033-math-assumptions-cleanup/research.md) (site inventories R1/R2 + decisions D1–D8), [`data-model.md`](./specs/033-math-assumptions-cleanup/data-model.md), [`contracts/assumptions.contract.md`](./specs/033-math-assumptions-cleanup/contracts/assumptions.contract.md), [`quickstart.md`](./specs/033-math-assumptions-cleanup/quickstart.md). Key locked decisions: `stockContribution` keeps v2 planned semantics (sibling-field lesson); brokerage draw at face value (no LTCG gross-up, D4); assumptions.js loads FIRST among calc scripts (global-scope lesson); static guards in `tests/unit/mathAssumptions.test.js` ban hardcoded multipliers + subtraction-form rates. Origin: external review 2026-06-05 (BUG-2/3/4 verified; BUG-1 did NOT reproduce post-032-hotfix). Merge gate: [`quickstart.md`](./specs/033-math-assumptions-cleanup/quickstart.md) incl. FIRE-age before/after per mode (expect later/lower — both corrections are conservative).
