...
| `strategy.aggressiveBracketFill.name` | Aggressive Bracket-Fill | 主動填滿稅階 |
| `strategy.aggressiveBracketFill.desc` | Quadruples the per-year smoothing cap so MORE Trad flows each year while still spreading evenly across retirement. Steps your effective tax rate up by ~1 bracket from Smoothed but stays inside the 12% bracket — never jumps to 22%. After-tax surplus reinvests into Taxable stocks. Lower lifetime tax + bigger estate than Smoothed for modest-Trad households. | 把每年的平滑上限調為 4 倍，讓退休每年提領更多 Trad，但仍然平均分配到整個退休期。實際稅率比「平滑填平」高約 1 個級距，但仍停留在 12% 級距內 — 不會跳到 22%。扣稅後的剩餘投回應稅股票。對於 401(k) 餘額不大的家庭：終身稅更低、遺產更多。 |
| `strategy.aggressiveBracketFill.narrative` | Strategy: aggressive bracket-fill — every accessible pre-SS year uses 4 × pTrad ÷ yearsRemaining as the per-year cap (still bounded by (stdDed + top12) × (1 − safetyMargin) so 22% bracket is never breached); after-tax surplus routes into stocks via synthetic conversion. Reverts to unscaled smoothed cap age 70+ when SS becomes ordinary income. | 策略：主動填滿稅階 — 每個可動用且尚未領取社安的退休年使用 4 × pTrad ÷ 剩餘年數 作為每年上限（仍受 (stdDed + top12) × (1 − 安全邊際) 限制，確保不會踩進 22% 級距）；扣稅後的剩餘透過合成轉換流入股票。社安開始（70 歲後）轉回未放大的平滑上限。 |
| `strategy.aggressiveBracketFill.tooltip` | Pick this when your Trad 401k balance is modest relative to retirement length and you want to use more of your 12% bracket headroom in years 60-69 (before SS taxable income stacks with Trad pulls). The Smoothed variant uses 1× cap and may leave deduction headroom unused; this variant uses 2× cap. Trad still spreads evenly across all retirement years — no concentration in 60-63. | 當你的 Trad 401(k) 相對於退休年數偏低、且想在 60–69 歲（社安開始前）善用 12% 級距空間時選擇這個策略。「平滑填平」用 1 倍上限可能讓扣除額空間沒有被用滿；本策略用 4 倍上限。Trad 仍然平均分散在所有退休年份 — 不會集中在 60–63 歲那幾年。 |
...
| `display.frame.countryTierFrameNote` | Cost in today's $; the dashboard inflates this to your retirement year for projections. | 成本以今日美元計算；儀表板會將其通膨調整至您的退休年份用於預測。 |

### Feature 023 — Accumulation-vs-Retirement Spend Separation

Added 2026-05-02 alongside `getAccumulationSpend(inp)` inline helper +
`resolveAccumulationOptions` extension. Both EN + zh-TW keys live in
`TRANSLATIONS.en` + `TRANSLATIONS.zh` of `FIRE-Dashboard.html` and
`FIRE-Dashboard-Generic.html` (lockstep). Distinguishes the two spending
concepts: "current spending" (drives accumulation phase) vs the country-tier
"annual budget" (drives retirement phase).

| Key | EN | zh-TW |
|---|---|---|
| `expenses.caption.currentSpending` | Current spending (US household, today's dollars) | 目前支出（美國家計，今日購買力） |
...
## Feature 034 — Year Tax Estimator (RR only)

New keys added in `TRANSLATIONS.en` and `TRANSLATIONS.zh` of **`FIRE-Dashboard.html` only**
(the estimator UI is a personalized MFJ tax-planning tool scoped to the RR dashboard;
`FIRE-Dashboard-Generic.html` loads `calc/taxEstimator.js` for calc-layer lockstep but has
no UI and no new strings — documented Principle-I divergence). Financial acronyms (LTCG,
IRMAA, NIIT, MFJ, Roth) stay English per the Principle VII exemption; surrounding prose
translates. User-facing copy uses "dollars / gains / tax owed", never "real $" (FR-022).

Wave 2a keys (block chrome + US1 room-left headline + US2 breakdown cards & steps). The
marginal / IRMAA / NIIT chip + tooltip keys land in a later wave (T023-T027).
...
| `te.input.rothConversion` | Roth conversion | Roth 轉換 |
| `te.input.ltcg` | Realized long-term capital gain (LTCG) | 已實現長期資本利得（LTCG） |
| `te.input.standardDeduction` | Standard deduction | 標準扣除額 |
...
Edit-mode helper text (T022), marginal/IRMAA/NIIT chips + tooltips (T023-T025), and the
`info-tip` label/card tooltips (T026-T027). Same scope: `FIRE-Dashboard.html` only.

| Key | EN | zh-TW |
|---|---|---|
| `te.editHelp` | Edit any value to explore a what-if — only this estimator updates. Hit “Reset to projection” to restore the auto-pulled numbers. | 編輯任何數值以探索假設情境 — 只有此估算器會更新。點擊「重設為預測值」即可還原自動帶入的數字。 |
| `te.marginal.ordinary` | Next ordinary $ taxed at | 下一元一般收入稅率 |
| `te.marginal.ltcg` | Next gain $ taxed at | 下一元利得稅率 |
| `te.marginal.tip` | The rate your NEXT dollar would pay. Adding ordinary income (a bigger Traditional withdrawal or Roth conversion) shrinks your 0% capital-gains room and can flip gains from 0% to 15%. | 你下一元將適用的稅率。增加一般收入（更大的 Traditional 提領或 Roth 轉換）會縮小 0% 資本利得空間，並可能把利得從 0% 推升至 15%。 |
| `te.irmaa.chip` | IRMAA Tier 1 crossed | 已超過 IRMAA Tier 1 |
| `te.irmaa.tip` | Your MAGI this year is above the IRMAA Tier 1 threshold, which raises your Medicare Part B & D premiums two years later. The threshold here is inflated to the selected year. | 你今年的 MAGI 高於 IRMAA Tier 1 門檻，兩年後會提高你的 Medicare Part B 與 D 保費。此處門檻已通膨調整至所選年份。 |
| `te.niit.chip` | NIIT 3.8% applies (+${0}) | NIIT 3.8% 適用（+${0}） |
| `te.niit.tip` | The 3.8% Net Investment Income Tax hits investment gains once MAGI tops $250K (MFJ). This threshold is NOT inflation-indexed — it stays a fixed $250K, so later years trigger it more easily. | 3.8% 淨投資所得稅（NIIT）在 MAGI 超過 $250K（MFJ）後對投資利得課徵。此門檻不會隨通膨調整 — 永遠固定為 $250K，因此後續年份更容易觸發。 |
| `te.tip.otherOrdinary` | The portion of income taxed at ordinary rates that isn't a Traditional withdrawal or Roth conversion — mainly the taxable 85% of Social Security plus any taxable interest. | 以一般稅率課稅、但非 Traditional 提領或 Roth 轉換的收入部分 — 主要是 Social Security 應稅的 85% 加上應稅利息。 |
