// V4 — verify Safe solver against chart rendering
...
console.log('=== SCAN: Safe feasibility by FIRE age ===');
for (let fa = 50; fa <= 60; fa++) {
  const s = api.signedLifecycleEndBalance(inp, spend, fa);
  const feas = api.isFireAgeFeasible(s, inp, spend, 'safe');
  console.log(`  FIRE ${fa}: endBal(signed)=$${Math.round(s.endBalance).toLocaleString()}, atUnlock=$${Math.round(s.balanceAtUnlock).toLocaleString()}, atSS=$${Math.round(s.balanceAtSS).toLocaleString()}, SafeFeas=${feas}`);
}

api.setFireMode('safe');
const sf = api.findFireAgeNumerical(inp, spend, 'safe');
console.log(`\nSafe solver: fireAge=${42+sf.years}, feasible=${sf.feasible}, endBal=$${Math.round(sf.endBalance).toLocaleString()}`);

const chart = api.projectFullLifecycle(inp, spend, 42+sf.years, true);
const at100 = chart.find(r => r.age === 100);
console.log(`Chart at 100: total=$${Math.round(at100?.total||0).toLocaleString()}`);
console.log(`Signed endBal at 100: $${Math.round(sf.endBalance).toLocaleString()}`);

const depleted = chart.find(r => r.age >= 42+sf.years && r.total < 10000);
console.log(`Chart reaches <$10K at age ${depleted ? depleted.age : 'never'}`);

let minT = Infinity, minA = 0;
for (const r of chart) if (r.age >= 42+sf.years && r.total < minT) { minT = r.total; minA = r.age; }
console.log(`Chart min after FIRE: $${Math.round(minT).toLocaleString()} at age ${minA}`);
