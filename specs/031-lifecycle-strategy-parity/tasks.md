...
- [X] T009 [P] [US2] Add `tests/e2e/lifecycle-strategy-parity-drag.spec.ts` (Playwright): with a non-default winner, drag the FIRE marker and assert the Lifecycle Trad series and the Withdrawal Strategy bars reflect the same strategy at the previewed age, and after release. Confirm it FAILS (or is skipped pending live server) against current code. **Done**: 3 tests × 2 dashboards. Confirmed RED pre-fix (RR C2 test failed $166K divergence; Generic non-discriminating — winner≡bracket-fill for those inputs, annotated).

### Implementation

- [X] T010 [US2] In the FIRE-marker drag-preview render (RR `:14911-14922`, esp. the `renderGrowthChart` call at `:14919`), thread the strategy resolved for `_previewFireAge` (or consistently preview one strategy across all three surfaces) so the lifecycle chart never threads a winner ranked at a different age. Apply identically in Generic (~`:15299-15310`). [lockstep] **Done**: pinned the lifecycle preview to bracket-fill while `_previewFireAge` is active (re-ranking per mousemove breaks the ≥30fps floor). Guard `_fireDragPreviewActive` in `renderGrowthChart` strategy-resolution block (RR `:16378`, Generic `:16770`). Byte-identical, +13/-2 in both files.
- [X] T011 [US2] Ensure the drag-commit path (mouseup → `recalcAll`) benefits from the US1 post-rank render (verify no separate stale render remains in the drag commit branch). Confirm in both files. [lockstep] **Done/verified**: commit promotes the preview via `chartState.setOverride` (the shared override path; same as reset button & mode switches). On commit `_previewFireAge` is cleared BEFORE setOverride fires, so the guard is inert and `onFireChange_growthChart` + `onFireChange_rothLadder` both re-render from the SAME `_lastStrategyResults` winner at the override age → surfaces stay mutually consistent (C2-on-commit). No drag-specific stale render exists. Instrumented: recalcAll does NOT run on the chartState confirm path (count stayed 0) — a full re-rank on every override gesture is a separate pre-existing architectural item, out of US2 scope.
- [X] T012 [US2] Run the drag E2E against a live server (`python -m http.server`) → GREEN for both dashboards. **Done**: ran via Playwright webServer (port 8766). 6/6 green with fix; RR C2 test goes RED without fix (TDD verified). Full `npm run test:unit` = 606/606, no regression.
