...
### B-035-1. Generic dashboard horizontal overflow at phone width (pre-existing content card)

**Surfaced during feature 035 (left-sidebar-nav) E2E, 2026-06-16.** At a ~390px viewport, `FIRE-Dashboard-Generic.html` has `document.documentElement.scrollWidth ≈ 658` (a horizontal scrollbar). Traced to a ~590px-wide `.card` in the Generic **Plan → Profile** panel (generic profile/income inputs — date pickers + household composition + college/loan summaries — whose min-content exceeds the viewport). **This is PRE-EXISTING, present on `main` before feature 035** (RR's equivalent card is 183px because it's the fixed two-person version, so RR does not overflow). Feature 035's nav layout itself is fully clamped (`min-width:0` on `#navLayout`; the off-canvas rail adds no width) and is NOT the cause — 035's SC-003 was scoped to the nav layer accordingly. Fix is a small content-layer change in BOTH files (lockstep): e.g. `min-width:0` on `.pill-host > .card` / the wide input grid, or let the Profile inputs wrap at narrow widths — verify it doesn't disturb the desktop Profile layout. Out of scope for the nav relocation; logged here for a focused follow-up.
