...
- [~] **Feature 037 — Year-by-Year Lifecycle Spreadsheet Export** (implemented 2026-08-13, branch `037-lifecycle-excel-export`, awaiting human Excel open — T040). One button in **History → Snapshots** downloads a real `.xlsx`: **one row per calendar year** from the current year to plan end, **70 columns**, every figure in both money (Book Value) and purchasing-power frames, matching the Lifecycle chart exactly. Answers "what is the number in 2043, and why?" — previously only reachable by hovering tooltips one point at a time. New pure UMD module `calc/lifecycleExport.js` (UMD const `_lifecycleExportApi`) holds all the logic and is Node-testable with no DOM and no library reference; a thin browser shim writes bytes. **Principle V exception, user-approved**: adds **ExcelJS 4.4.0** (cdnjs, MIT, 926 KB, verified classic UMD / zero ESM syntax) — the first runtime dependency beyond Chart.js — **lazy-loaded on first click** so cold load is unchanged and non-exporters pay nothing. SheetJS CE was rejected (freeze panes are Pro-only). The export reads the *cached* chart projection and the *active* strategy — never a recompute — so it cannot drift from the chart. **Zero calc-behaviour changes**: the planned one was cancelled when `mix.taxOwed` turned out to already exist on rows the export was already joining; the single additive field (`signedTotal`, the un-clamped total) is read only by the export. Structural finding: "all the numbers" is a **union of three per-year arrays joined by `age`**, with **blank ≠ zero** semantics. 12 bilingual i18n keys + **all 70 column headers localised**. Tests: unit **818/818**; full Playwright **220/1** (the 1 passes 10/10 isolated — load flake, see BASELINE.md); 037 E2E **20/20** both dashboards; `tools/verify-037-export.mjs` **42/42** against live RR numbers (**53/53 years match** the rendered chart, RR 2026 = $532,021); console-probe 0 on both; smoke-032 15/15. Spec/plan/tasks/CLOSEOUT: [specs/037-lifecycle-excel-export/](./specs/037-lifecycle-excel-export/).
- [~] **Feature 036 — Explicit Retirement Status** (implemented 2026-07-02, branch `036-retirement-status`, awaiting full-suite + human smoke). Separates *when the user CAN retire* (the Safe/Exact/DWZ feasibility scan) from *when the user HAS retired* (a durable user-asserted fact). New **"I've retired"** switch + retirement **year**: when ON, the actual retirement age becomes the accumulation→drawdown transition threaded through the single `effectiveFireAge` slot (`fireAgeOverride ?? calculatedFireAge`, re-pinned inside `_setCalculatedFire` so it survives every recalc pass — Principle III), superseding the FIRE-marker drag (drag gated inert, FR-011). Stops all employment income + new contributions from the retirement year; SS/passive stay on `ssClaimAge` (FR-004). (US2) verdict reframes from "FIRE in N years" countdown to a sustainability readout ("🟢 sustainable to age N" / "⚠️ at risk — shortfall in {year}"), reusing the strategy-aware `projectFullLifecycle` probe + per-year `hasShortfall` (FR-006/FR-014). (US4) session-scoped dismissible auto-suggest nudge when feasible today. (US5, Generic only) per-person staggered retirement + new `person1Income`/`person2Income` inputs (household = sum) driving `calc/accumulateToFire.js` v8's new `options.retirement.households[]` per-year income masking + proportional contribution scaling — a deliberate Principle-I divergence (RR keeps one household date). Persistence: additive `state._retirementStatus` (no `GENERIC_VERSION` bump — would wipe data). 12 new bilingual i18n keys × EN + zh-TW. Functional probe verified on both dashboards: retire-now pins the transition age, early retirement honestly shows the shortfall year, OFF reverts exactly, reload persists (errorCount 0). Spec/plan/tasks: [specs/036-retirement-status/](./specs/036-retirement-status/).
- [~] **Feature 035 — Left-Sidebar Navigation** (implemented 2026-06-16, branch `035-left-sidebar-nav`, awaiting human visual smoke + merge). Relocates the primary tab group (`#tabBar`) + the per-tab pill-bars out of the top sticky stack into a persistent, sticky left sidebar (`#navRail`) rendered as an **accordion** (active tab expands to reveal its pills); the header + mode/Withdraw-Strategy band (`#gateSelector`) **stay on top** (clarified — only 2 of the 3 rows move). Mobile: **hamburger drawer** (`#navDrawerToggle` + `#navScrim`, off-canvas at ≤767px, `body.nav-drawer-open` overlay z-65 that opens *below* the header band so every tab is clickable). **No button-behavior change** — navigation stays owned by `window.tabRouter`; only its `pillBarsByTab` selectors + an `onAfterActivate` accordion hook (`#navRail[data-active-tab]`) were rewired; `.pill-host` content untouched. New CSS var `--navrail-width` (distinct from the audit `--sidebar-width`); 1 new bilingual i18n key (`nav.drawerToggle`). Both dashboards, lockstep (9 markers byte-matched). Tests: full Playwright **185/185** (new `tests/e2e/left-sidebar-nav.spec.ts` 18/18 + `tab-navigation.spec.ts` 33/33, with 8 stale horizontal-mobile-bar fixtures updated to the accordion/drawer model) + unit suite unchanged green + console-probe 0 on both. Out-of-scope finding tracked separately: a **pre-existing** ~590px Generic Plan→Profile content card overflows at phone width (present on `main`, not caused by the nav). Spec/plan/tasks: [specs/035-left-sidebar-nav/](./specs/035-left-sidebar-nav/).
- [x] **Feature 034 — Year Tax Estimator** (merged to main 2026-06-16, branch `034-year-tax-estimator`). RR-only single-year federal tax microscope at the bottom of the Withdrawal Strategy tab: year picker → auto-pull projected ordinary income + LTCG (real→nominal) → LTCG-on-ordinary stacking with **standard-deduction shelter** (`roomLeftAt0 = max(0, ceiling + stdDed − gross − ltcg)`; matches the IRS QDCGT worksheet / the ~$123,250 MFJ tax-free-gains figure), "Room left at 0%" + "Ordinary-income headroom" headlines, marginal/IRMAA/NIIT chips, a methodology ⓘ, and show-your-work cards (deduction-shelter + 0%-pool steps). Indexed thresholds shown in the **selected year's nominal dollars, used as-is** (year-stamped labels). Pure UMD `calc/taxEstimator.js` in both files; UI RR-only (Principle-I divergence). Includes the deduction-shelter correctness fix + year-frame consistency follow-up wave. Tests: **735/735 unit + 5/5 E2E**. Spec: [specs/034-year-tax-estimator/](./specs/034-year-tax-estimator/).
- [x] **Feature 033 — Math-Assumptions Cleanup** (implemented 2026-06-06, branch `033-math-assumptions-cleanup`). Three corrections to the projection math, one wave, full attribution. (US1) New `calc/assumptions.js` — THE single defining location for cross-simulator assumptions: `CASH_REAL_RETURN = 0.0` (cash now tracks inflation; was 0.5% above it forever, hardcoded at 9 sites per HTML + `calc/accumulateToFire.js` + a `returnRateCashReal` mirror in `calc/getCanonicalInputs.js`) and the Fisher helper `realRate(nominal, inflation) = (1+n)/(1+i)−1`. Static guards in `tests/unit/mathAssumptions.test.js` ban hardcoded multipliers and subtraction-form rates from ever returning. (US2) Honest shortfall funding ladder in `accumulateToFire` v7: negative accumulation residuals now fund via cut stock contribution → draw cash → draw brokerage instead of silently flooring to $0 — the audit's cash-flow conservation residual went from ≈ −$32K to $0 (per-year identity I6 verified in the live browser); new sibling row fields (`stockContributionActual`, `fundedFromCash`, `fundedFromStocks`) + bilingual `CONTRIBUTION_REDUCED` info flag; conservation block v4. (US3) Fisher real rates across 28 sites per HTML + 4 calc sites incl. SS-COLA (exact no-op on defaults) and income growth. **Net effect on RR cold-load defaults: FIRE age unchanged at 50 in all three modes; end balance −$369,616 (−43.7%, $845,459 → $475,843) — phantom money removed, $239K by the ladder + $131K by Fisher.** Four pre-existing verdict-consistency bugs exposed by the lower honest trajectory and fixed: DWZ month-interpolation accepting gate-rejected fractional ages, the DWZ gate's vacuous clamped-endBalance check (now also requires signed ≥ 0), `yearsToFIRE` flooring the month-precise result, and `getCanonicalInputs.js`'s ESM-incompatible require guard silently binding undefined. Deferred: B-033-1 (LTCG gross-up on the brokerage rung), B-033-2 (user-facing cashRealReturn input), B-033-3 (signed check for Safe/Exact gates). Tests: **697/697 unit + 163/163 Playwright E2E**; zero non-expected cross-validation warnings in all three modes. Spec: [specs/033-math-assumptions-cleanup/spec.md](./specs/033-math-assumptions-cleanup/spec.md) · CLOSEOUT: [specs/033-math-assumptions-cleanup/CLOSEOUT.md](./specs/033-math-assumptions-cleanup/CLOSEOUT.md).
- [x] **Feature 032 — Roth IRA Accounts (Roger & Rebecca)** (merged to main 2026-06-05 with automated browser smoke 15/15 + full E2E 163/163; implemented 2026-05-28, browser smoke pending, branch `032-roth-ira-accounts`). Introduces a distinct Roth IRA pool ($pRothIra$) on the RR dashboard alongside the existing Roth 401K pool, fixing the long-standing conflation where the canonical `rothIraReal` field actually held Roth 401K data (renamed to `roth401kReal` per research.md Q1). (US1) New "🔒 Roth IRA" card on Plan→Assets tab with two locked-pool inputs — Roger's Roth IRA (default $0) and Rebecca's Roth IRA (default $59,021) — wired through localStorage persistence + bilingual i18n. Header net-worth total + "Locked" sub-label include the new pool. (US2) Lifecycle chart gains a dedicated "Roth IRA" stacked-area series (lighter purple `--chart-rothIra: #a890ff`) with `pRothIra` + `pRothIraBookValue` carried on every row, growing through accumulation and depleting per the active strategy. (US3) **CRITICAL FR-021e**: `effBal()` extended to include `pRothIra` so the Safe/Exact/DWZ verdict gates evaluate the same balance the chart renders — closes a feature-031-class drift risk. Signed-sim fallback (`simulateRetirementOnlySigned`) gains a `p401kRothIra0` parameter threaded from `inp.rothIraReal`. (US4) `POOL_KEYS` extended with `'rothIra'` immediately after `'roth'`; every `STRATEGY_ORDERS` row drains Roth IRA in that position. RMD branch remains `trad`-only (FR-021d). Withdrawal Strategy tooltip surfaces a dedicated `rothIra` pool line via `calc/withdrawalTooltipFrame.js`. (US4b) Two Investment-tab number inputs ("Roger's Roth IRA Contribution" / "Rebecca's Roth IRA Contribution", default $7,000 each — matching 2026 IRS limit) grow the pool annually via `rothIraContrib` in the accumulation loop. (US5) Snapshot CSV schema bumped append-only with `rogerRothIra` + `rebeccaRothIra` columns; legacy short-row tolerance keeps pre-feature snapshots loading. History tab table renders the new columns. (US6) Copy-debug snapshot emits `pRothIra` in both real-$ and book-value frames; audit composition surfaces `lockedRothIra` alongside `lockedRoth401k`; all 6 audit invariants (A–F) pass on the new persona variant. Constitution Principle I lockstep: every calc-layer edit landed byte-equivalent in `FIRE-Dashboard-Generic.html` even though the Generic UI is untouched per FR-018 (Generic's `rothIraReal` defaults to 0 because no UI feeds it). 18 new bilingual i18n keys × EN + zh-TW. Tests: **666/666 unit** (661 baseline + 3 integration + 2 orthogonality) + 10/10 Playwright (6 existing drag E2E + 4 new Roth IRA flow). Spec: [specs/032-roth-ira-accounts/spec.md](./specs/032-roth-ira-accounts/spec.md) · Plan: [specs/032-roth-ira-accounts/plan.md](./specs/032-roth-ira-accounts/plan.md) · Tasks: [specs/032-roth-ira-accounts/tasks.md](./specs/032-roth-ira-accounts/tasks.md) · Quickstart: [specs/032-roth-ira-accounts/quickstart.md](./specs/032-roth-ira-accounts/quickstart.md).
- [~] **Feature 031 — Lifecycle Strategy Parity** (implemented 2026-05-27, browser smoke pending, branch `031-lifecycle-strategy-parity`). Fixes the bug where the Lifecycle chart's Trad-401K line only dropped after RMD age (~72) while the Withdrawal Strategy chart drew Trad from ~60. Root cause (user's hypothesis, confirmed): the Lifecycle chart rendered via the `chartState.onChange` listener inside `_setCalculatedFire` **before** `scoreAndRank` resolved the winner, and got no post-rank re-render, so it silently drew default bracket-fill while every other surface drew the winning strategy. (US1) post-rank `renderGrowthChart` re-render mirroring the existing post-rank `renderRothLadder` — the Lifecycle chart now consumes the resolved winner. (US2) FIRE-marker drag pins the lifecycle preview to a consistent basis (no per-pixel re-rank; preserves ≥30fps); commit reconciles via `setOverride`. (US3) Safe/Exact/DWZ verdict re-evaluated under the displayed winner post-rank (Option ii — no circularity; FIRE-age search stays deterministic); removed `_invariantA` strategy-mismatch suppression. (US4) Withdrawal Strategy tooltip single-frame reconciliation via new pure `calc/withdrawalTooltipFrame.js` helper + bilingual `tw.tooltip.purchasingPower`. (US5) cash-sweep parity — `_applyCashSweep` added to `projectFullLifecycle`'s retirement loop matching the other 5 sites. Tests: 614/614 unit (606 baseline + 6 tooltip + 2 sweep structural) + 6/6 live Playwright drag E2E; review gates 6 & 7 green. Spec: [specs/031-lifecycle-strategy-parity/spec.md](./specs/031-lifecycle-strategy-parity/spec.md) · Plan: [specs/031-lifecycle-strategy-parity/plan.md](./specs/031-lifecycle-strategy-parity/plan.md) · Tasks: [specs/031-lifecycle-strategy-parity/tasks.md](./specs/031-lifecycle-strategy-parity/tasks.md).
- [~] **Feature 018 — Lifecycle Payoff Merge** (implemented 2026-04-29, browser smoke pending, branch `018-lifecycle-payoff-merge`). Unifies Payoff-vs-Invest with the Full Portfolio Lifecycle chart so the dashboard tells one coherent story. (1) US1 — `pviLumpSumPayoff` checkbox replaced by 3-option mortgage strategy radio (Prepay / Invest-keep-paying / Invest-lump-sum); new `getActiveMortgageStrategyOptions()` helper threads `mortgageStrategyOverride` to every `projectFullLifecycle` call site (chart + FIRE-age search + audit + ranker), mirroring feature 008's `getActiveChartStrategyOptions()` pattern. Lifecycle simulator's pre-FIRE accumulation reads strategy-aware monthly P&I from the calc module's per-year `amortizationSplit`. (2) US4 — Sell-at-FIRE × strategy composition: new `homeSaleEvent` and `postSaleBrokerageAtFire[strategy]` outputs with Section 121 exclusion modeled (MFJ $500K / single $250K). Lump-sum trigger inhibited at age ≥ FIRE under `sellAtFire=true`. PvI brokerage chart shows green-star sell marker; verdict banner Line 4 surfaces sale proceeds; lifecycle truncates mortgage at FIRE and seeds retirement-phase brokerage from `postSaleBrokerageAtFire[strategyKey]`. (3) US2 — Sidebar `#sidebarMortgageStatus` indicator surfacing active strategy + payoff age. (4) US3 — Verdict + ranker auto-react: radio-change clears `fireAgeOverride`, audit `subSteps[]` flow verbatim from calc (5 new v3 strings), `copyDebugInfo()` extended with mortgage-strategy fields plus `feasibilityProbe.activeMortgageStrategy` for LH-Inv-1 verifiability. Calc v3: `LumpSumEvent` typedef gains `actualDrawdown` field (Option B — `paidOff` retains v2 semantics; `actualDrawdown = paidOff × (1 + ltcgRate × stockGainPct)` is the true brokerage drop including LTCG gross-up); trigger threshold corrected to `investedI >= actualDrawdown` so brokerage cannot go negative. 11 new bilingual i18n keys (EN + zh-TW). Tests: 51/51 fixture + 4/4 lifecycle handoff = 55/55 pass (up from v017's 43/43). Spec: [specs/018-lifecycle-payoff-merge/spec.md](./specs/018-lifecycle-payoff-merge/spec.md) · Plan: [specs/018-lifecycle-payoff-merge/plan.md](./specs/018-lifecycle-payoff-merge/plan.md) · Tasks: [specs/018-lifecycle-payoff-merge/tasks.md](./specs/018-lifecycle-payoff-merge/tasks.md) · CLOSEOUT: [specs/018-lifecycle-payoff-merge/CLOSEOUT.md](./specs/018-lifecycle-payoff-merge/CLOSEOUT.md).
- [~] **Feature 014 — Calculation Audit View** (implemented, browser smoke pending, branch `014-calc-audit`). Adds a 5th top-level tab labeled **Audit** that exposes every step of the lifecycle pipeline as both per-section charts AND tables. Opens with a horizontal Calculation-Flow Diagram (Inputs → Spending Adjustments → Gate Evaluations → FIRE Age Resolution → Strategy Ranking → Lifecycle Projection) where each stage shows a one-line headline output and clicking scrolls to its detail section. The 7 detail sections are: Resolved Inputs (composition pie + table), Spending Adjustments (effective spend curve), Gate Evaluations (Safe / Exact / DWZ each with trajectory chart + floor + violation list), FIRE Age Resolution (candidate-age scatter), Strategy Ranking (grouped bar chart with all 7 strategies side-by-side), Lifecycle Projection (chart thumbnail + scrollable per-year table), Cross-Validation Warnings (auto-flags 4 invariants when calc paths diverge). Pure observability layer — **zero calc-engine modifications** (verified by SC-008: zero diff hunks in 12 named calc functions on this branch vs main). New pure module `calc/calcAudit.js` (824 lines, 16 unit tests) exposes `assembleAuditSnapshot(options) → AuditSnapshot` consumed by both the UI renderers AND the Copy Debug serializer (which gains a top-level `audit` key alongside the existing `feasibilityProbe` / `summary` / `lifecycleSamples` — purely additive per FR-020). 44 new bilingual i18n key pairs × EN + zh-TW. Lockstep verified across 9 structural metrics in both HTML files. New 45-test Playwright spec (`tests/e2e/calc-audit.spec.ts`) covers all 4 user stories + lockstep DOM-diff — all green alongside Features 011's 13 + 013's 35 + 014's 2 file:// tests = 95-test E2E suite. 211 unit tests green (195 baseline + 16 new). Reuses Chart.js (no new chart library); flow diagram is HTML+CSS only (no library). Spec: [specs/014-calc-audit/spec.md](./specs/014-calc-audit/spec.md) · Plan: [specs/014-calc-audit/plan.md](./specs/014-calc-audit/plan.md) · Tasks: [specs/014-calc-audit/tasks.md](./specs/014-calc-audit/tasks.md).
